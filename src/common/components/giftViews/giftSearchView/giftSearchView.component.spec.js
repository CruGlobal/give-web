...
describe('giftSearchView', () => {
  ...
  describe('searchHandler', () => {
    beforeEach(() => {
      jest.spyOn(self.controller.designationsService, 'productSearch').mockReturnValue(Observable.empty())
      ...
      jest.spyOn(Observable.prototype, 'debounceTime').mockImplementation(function (dueTime) { // Don't use arrow function here for this to be defined correctly
    ...
    it('should submit a query and load the result array', () => {
      self.controller.designationsService.productSearch.mockReturnValue(Observable.of([
    ...
    it('should submit a query and handle an error response', () => {
      self.controller.designationsService.productSearch.mockReturnValue(Observable.throw('some error'))
      self.controller.searchSubject.next('Joe')
      self.scheduler.flush()
    ...
    it('should only submit the last query when the debounce interval expires', () => {
      ...
      expect(self.controller.designationsService.productSearch).toHaveBeenCalledWith({ keyword: 'Joe' })
      expect(self.controller.designationsService.productSearch.mock.calls.length).toEqual(1)
    })
  ...
  describe('onSearchChange', () => {
    beforeEach(() => {
      jest.spyOn(self.controller.searchSubject, 'next').mockImplementation(() => {})
    })
