...
class GiftSearchViewController {
    ...
    this.searchSubject
      ...
      .switchMap(query => {
        ...
        if(query){
          ...
          this.$scope.$apply(() => this.searchState = 'initial');
          return Observable.empty();
        }
      })
      .subscribe( results => {
          if ( results.length ) {
            this.results = map( results, result => ({designationNumber: result.designationNumber, designationName: result.name}));
        ...
        error => {
          ...
          this.$log.error('Error loading search results in giftSearchView', error);
        });
  }
  ...
  gatherSelections(gift){
    if(this.selectable === 'radio'){
      ...
      this.onSelection({ selectedRecipient: find(this.results, {_selectedGift: true}) });
    }else{
      this.onSelection({ selectedRecipients: filter(this.results, {_selectedGift: true}) });
    }
  }
}
