...
describe('Recaptcha component', () => {
  ...
  beforeEach(() => {
    ...
    mockRecaptchaReady.mockImplementation((callback) => {
      callback();
    });
    onSuccess.mockClear();
  });
  ...
  it('should render', () => {
    const { getAllByRole } = render(buildRecaptcha());
    ...
    expect((recaptchaEnabledButton as HTMLButtonElement).disabled).toEqual(
      false,
    );
  ...
  it('should store the recaptcha token and action', async () => {
    ...
    await waitFor(() => {
      ...
      expect(window.sessionStorage.getItem('recaptchaAction')).toEqual(
        'checkout',
      );
    });
  });
  ...
  it('should not block the gift if something went wrong with recaptcha', async () => {
    ...
    await waitFor(() => {
      ...
      expect($log.error).toHaveBeenCalledWith(
        'Failed to verify recaptcha, continuing on: Failed',
      );
    });
  });
