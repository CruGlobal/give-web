...
describe('navCart', () => {
  ...
  describe('loadCart', () => {
    ...
    it('should load the cart data', () => {
      $ctrl.cartService.get.mockReturnValue(
        Observable.of({ items: ['first item'] }),
      );
  ...
  describe('checkout', () => {
    it('should redirect to sign-in', () => {
      ...
      expect($ctrl.$window.location).toBe(
        'https://give-stage2.cru.org/sign-in.html',
      );
    });
    ...
    it('should redirect to checkout', () => {
      ...
      expect($ctrl.$window.location).toBe(
        'https://give-stage2.cru.org/checkout.html',
      );
    });
  });
  ...
  describe('buildCartUrl', () => {
    it('should build a url without query parameters', () => {
      jest
        ...
        .mockReturnValue('cart.html');
      expect($ctrl.buildCartUrl()).toEqual(
        'https://give-stage2.cru.org/cart.html',
      );
    });
    ...
    it('should build a url with query parameters', () => {
      jest
        .spyOn($ctrl.cartService, 'buildCartUrl')
        .mockReturnValue('cart.html?one=1&two=2');
      expect($ctrl.buildCartUrl()).toEqual(
        'https://give-stage2.cru.org/cart.html?one=1&two=2',
      );
    });
  });
});
