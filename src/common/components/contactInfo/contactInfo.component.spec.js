...
describe('contactInfo', function () {
  ...
  describe('addCustomValidators()', () => {
    it('should create validators', () => {
      ...
      expect(
        self.controller.detailsForm.phoneNumber.$validators.phone(
          '541-967-0010',
        ),
      ).toEqual(true);
      expect(
  ...
  describe('loadDonorDetails', () => {
    beforeEach(() => {
      jest
        .spyOn(self.controller, 'loadRadioStations')
        .mockImplementation(() => {});
    });

    it("should get the donor's details", () => {
      jest
        .spyOn(self.controller.orderService, 'getDonorDetails')
        .mockImplementation(() =>
          Observable.of({
    ...
    it("should disable name fields if the user's registration state is completed", () => {
      jest
        .spyOn(self.controller.orderService, 'spouseEditableForOrder')
      ...
      jest
        .spyOn(self.controller.orderService, 'getDonorDetails')
        .mockImplementation(() => Observable.of(donorDetails));
      self.controller.loadDonorDetails();
      ...
      expect(
        self.controller.orderService.spouseEditableForOrder,
      ).toHaveBeenCalledWith(donorDetails);
    ...
    it('should set the donor type if it is an empty string', () => {
      jest
        .spyOn(self.controller.orderService, 'getDonorDetails')
        .mockImplementation(() =>
          Observable.of({ 'donor-type': '', 'spouse-name': {} }),
        );
      self.controller.loadDonorDetails();
    ...
    it('should override details if passed in', () => {
      jest
        .spyOn(self.controller.orderService, 'getDonorDetails')
        .mockImplementation(() => Observable.of({ 'spouse-name': {} }));
    ...
    describe('pre-populate with overrideDonorDetails', () => {
      ...
      beforeEach(() => {
        jest
          .spyOn(self.controller.orderService, 'getDonorDetails')
          .mockImplementation(() => Observable.of(cloneDeep(initDonorDetails)));
        jest
          .spyOn(self.controller, 'waitForFormInitialization')
          .mockImplementation(() => {});
      });
      ...
      it('should overwrite donorDetails with overrideDonorDetails on intital load', () => {
        self.controller.$window.sessionStorage.removeItem(
          'initialLoadComplete',
        );
        ...
        expect(self.controller.donorDetails.name['given-name']).toEqual(
          'Test First Name',
        );
        expect(self.controller.donorDetails.name['middle-initial']).toEqual(
          'initial',
        );
        expect(self.controller.donorDetails.name['family-name']).toEqual(
          'Test Last Name',
        );
        ...
        expect(self.controller.donorDetails['donor-type']).toEqual(
          'Organization',
        );
        expect(
          self.controller.donorDetails['spouse-name']['given-name'],
        ).toEqual('Julie');
        expect(
          self.controller.donorDetails['spouse-name']['family-name'],
        ).toEqual('Smith');

        expect(self.controller.donorDetails.mailingAddress.country).toEqual(
          'US',
        );
        expect(
          self.controller.donorDetails.mailingAddress.streetAddress,
        ).toEqual('1111 Street Name');
        expect(
          self.controller.donorDetails.mailingAddress.extendedAddress,
        ).toEqual('Apartment 2');
        expect(self.controller.donorDetails.mailingAddress.locality).toEqual(
          'City',
        );
        expect(self.controller.donorDetails.mailingAddress.region).toEqual(
          'Georgia',
        );
        expect(self.controller.donorDetails.mailingAddress.postalCode).toEqual(
          '12345',
        );

        expect(
          self.controller.$window.sessionStorage.getItem('initialLoadComplete'),
        ).toEqual('true');
      });

      it('should not use overrideDonorDetails if initialLoadComplete is set', () => {
        self.controller.$window.sessionStorage.setItem(
          'initialLoadComplete',
          'true',
        );
        self.controller.donorDetails = overrideDonorDetails;
        ...
        expect(self.controller.donorDetails.email).toEqual(
        ...
        expect(
          self.controller.donorDetails.mailingAddress.locality,
        ).toBeUndefined();
        expect(
          self.controller.donorDetails.mailingAddress.region,
        ).toBeUndefined();
        expect(
  ...
  it('should override details with checkout saved data', () => {
    ...
    expect(self.controller.donorDetails['donor-type']).toEqual(
      checkoutData['donor-type'],
    );
    expect(self.controller.donorDetails.name['given-name']).toEqual(
      checkoutData.name['given-name'],
    );
    expect(self.controller.donorDetails['spouse-name']).toEqual(
      checkoutData['spouse-name'],
    );
    expect(self.controller.donorDetails.mailingAddress.streetAddress).toEqual(
      checkoutData.mailingAddress.streetAddress,
    );
  ...
  describe('loadRadioStations', () => {
    ...
    it('should not load if not requesting radio station', () => {
      ...
      jest
        .spyOn(self.controller.radioStationsService, 'getRadioStations')
        .mockImplementation(() => Observable.of([]));
      self.controller.loadRadioStations();

      expect(
        self.controller.radioStationsService.getRadioStations,
    ...
    it('should load if requesting radio station and postal code selected', () => {
      ...
      jest
        .spyOn(self.controller.radioStationsService, 'getRadioStations')
        .mockImplementation(() => Observable.of(radioStations));
      self.controller.loadRadioStations();

      expect(
        self.controller.radioStationsService.getRadioStations,
      ).toHaveBeenCalledWith(radioStationApiUrl, postalCode);
    ...
    it('should prepopulate the radio station name previously selected', () => {
      ...
      jest
        ...
        .mockImplementation(() => Observable.of(radioStations));
      jest
        .spyOn(self.controller.orderService, 'retrieveRadioStationName')
        .mockReturnValue('Radio Station');
      self.controller.loadRadioStations();
  ...
  describe('onSelectRadioStation', () => {
    ...
    it('should find selected radio station in list', () => {
      ...
      expect(self.controller.radioStationData).toEqual({
        WXYZ: 'Radio Station',
      });
    });
  });
  ...
  describe('submitDetails', () => {
    ...
    it('should call onSubmit binding if there are errors', () => {
      ...
      jest
        ...
        .mockImplementation(() => {});
      jest
        .spyOn(self.controller.orderService, 'storeRadioStationData')
        .mockImplementation(() => {});
      ...
      expect(
        self.controller.orderService.storeRadioStationData,
      ).not.toHaveBeenCalled();
    ...
    it('should save donor details and email', () => {
      ...
      jest
        .spyOn(self.controller.orderService, 'updateDonorDetails')
        .mockReturnValue(Observable.of('donor details success'));
      jest
        .spyOn(self.controller.orderService, 'addEmail')
        .mockReturnValue(Observable.of('email success'));
      jest
        .spyOn(self.controller.analyticsFactory, 'checkoutStepOptionEvent')
        .mockImplementation(() => {});
      jest
        .spyOn(self.controller.orderService, 'storeRadioStationData')
        .mockImplementation(() => {});
      ...
      expect(self.controller.orderService.addEmail).toHaveBeenCalledWith(
        'someone@asdf.com',
        '/emails/crugive',
      );
      expect(self.controller.onSubmit).toHaveBeenCalledWith({ success: true });
      expect(
        self.controller.analyticsFactory.checkoutStepOptionEvent,
      ).toHaveBeenCalledWith(
        self.controller.donorDetails['donor-type'],
        'contact',
      );
    });
    ...
    it('should save radio station', () => {
      ...
      expect(
        self.controller.orderService.storeRadioStationData,
      ).toHaveBeenCalledWith(radioStationData);
    });
    ...
    it('should handle an error saving donor details', () => {
      ...
      jest
        .spyOn(self.controller.orderService, 'updateDonorDetails')
        .mockReturnValue(Observable.throw({ data: 'donor details error' }));
      self.controller.submitDetails();
      ...
      expect(self.controller.$log.warn.logs[0]).toEqual([
        'Error saving donor contact info',
        { data: 'donor details error' },
      ]);
    ...
    it('should handle an error saving email', () => {
      ...
      jest
        .spyOn(self.controller.orderService, 'addEmail')
        .mockReturnValue(
          Observable.throw({ data: 'Invalid email address: a@a' }),
        );
      self.controller.submitDetails();
      ...
      expect(self.controller.$log.warn.logs[0]).toEqual([
        'Error saving donor contact info',
        { data: 'Invalid email address: a@a' },
      ]);
