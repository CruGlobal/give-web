  ...
  waitForSecurityCodeInitialization() {
    const unregister = this.$scope.$watch(
      ...
      () => {
        ...
        this.creditCardPaymentForm.securityCode.$validators.cardTypeLength = (
          number,
        ) =>
          cruPayments.creditCard.cvv.validate.cardTypeLength(
            number,
            cruPayments.creditCard.card.info.type(
              this.creditCardPayment.cardNumber,
            ),
          );
  ...
  addCustomValidators() {
    const editAllowEmpty = (validator) => {
      return (number) =>
        ...
        validator(number); /* eslint-disable-line no-mixed-operators */
    };
    this.creditCardPaymentForm.cardNumber.$validators.minLength =
      editAllowEmpty(cruPayments.creditCard.card.validate.minLength);
    this.creditCardPaymentForm.cardNumber.$validators.maxLength =
      editAllowEmpty(cruPayments.creditCard.card.validate.maxLength);
    this.creditCardPaymentForm.cardNumber.$validators.knownType =
      editAllowEmpty(cruPayments.creditCard.card.validate.knownType);
    this.creditCardPaymentForm.cardNumber.$validators.typeLength =
      editAllowEmpty(cruPayments.creditCard.card.validate.typeLength);
    this.creditCardPaymentForm.cardNumber.$validators.checksum = editAllowEmpty(
      cruPayments.creditCard.card.validate.checksum,
    );

    this.creditCardPaymentForm.expiryMonth.$validators.expired = (
      expiryMonth,
    ) => {
      // Valid if a year has not been chosen or date is valid
      return (
        !this.creditCardPayment.expiryYear ||
        cruPayments.creditCard.expiryDate.validate.month(
          expiryMonth,
          this.creditCardPayment.expiryYear,
        )
      );
    ...
    this.creditCardPaymentForm.expiryYear.$validators.expired =
      cruPayments.creditCard.expiryDate.validate.year;
  ...
  savePayment() {
    ...
    if (this.creditCardPaymentForm.$valid) {
      ...
      const tokenObservable =
        this.paymentMethod && !this.creditCardPayment.cardNumber
          ? Observable.of({
          ...
          : this.tsysService.getManifest().mergeMap((data) => {
              ...
              const ccpEnvironment = productionEnvironments.includes(
                actualEnvironment,
              )
                ? 'production'
                : actualEnvironment;
              ...
              return cruPayments.creditCard.encrypt(
                this.creditCardPayment.cardNumber,
                this.hideCvv ? null : this.creditCardPayment.securityCode,
                this.creditCardPayment.expiryMonth,
                this.creditCardPayment.expiryYear,
              );
            });
        ...
        (tokenObj) => {
          this.onPaymentFormStateChange({
            $event: {
              ...
              payload: {
                creditCard: {
                  address: this.useMailingAddress
                    ? this.mailingAddress
                    : this.creditCardPayment.address,
                  'card-number': tokenObj.tsepToken,
                  'card-type':
                    this.cardInfo.type(this.creditCardPayment.cardNumber) ||
                    (this.paymentMethod &&
                      this.paymentMethod[
                        'card-type'
                      ]) /* eslint-disable-line no-mixed-operators */,
      ...
      this.analyticsFactory.handleCheckoutFormErrors(
