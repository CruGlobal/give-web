...
describe('credit card form', () => {
  ...
  beforeEach(inject(($rootScope, $httpBackend, $compile) => {
    ...
    const compiledElement = $compile(
      angular.element(
        '<credit-card-form payment-form-state="paymentFormState" on-payment-form-state-change="onPaymentFormStateChange($event)"></credit-card-form>',
      ),
    )(self.outerScope);
  ...
  describe('savePayment', () => {
    beforeEach(() => {
      ...
      jest
        ...
        .mockImplementation(() => {});
      jest
        .spyOn(self.controller.tsysService, 'getManifest')
        .mockReturnValue(
          Observable.of({ deviceId: '<device id>', manifest: '<manifest>' }),
        );
      jest.spyOn(cruPayments.creditCard, 'init').mockImplementation(() => {});
      jest.spyOn(cruPayments.creditCard, 'encrypt').mockReturnValue(
        Observable.of({
          tsepToken: 'YfxWvtXJxjET5100',
    ...
    it('should send a request to save the credit card payment and billing address info', () => {
      ...
      expect(cruPayments.creditCard.init).toHaveBeenCalledWith(
        'development',
        '<device id>',
        '<manifest>',
      );
      expect(cruPayments.creditCard.encrypt).toHaveBeenCalledWith(
        '4111111111111111',
        '123',
        12,
        2019,
    ...
    it('should handle an error retrieving the manifest from TSYS', () => {
      self.formController.$valid = true;
      self.controller.tsysService.getManifest.mockReturnValue(
    ...
    it('should handle an error while tokenizing the card', () => {
      self.formController.$valid = true;
      cruPayments.creditCard.encrypt.mockReturnValue(
        Observable.throw('some error'),
      );
      self.controller.savePayment();
  ...
  describe('form controller', () => {
    ...
    describe('cardNumber input', () => {
      ...
      it('should be valid if the field is empty and an existing payment method is present', () => {
        ...
        expect(
          self.formController.cardNumber.$error.cardNumber,
        ).toBeUndefined();
      });
      ...
      it('should not be valid if the checksum is invalid', () => {
        ...
        expect(self.formController.cardNumber.$error.knownType).toBeUndefined();
        expect(
          self.formController.cardNumber.$error.typeLength,
        ).toBeUndefined();
    ...
    describe('cardholderName input', () => {
      it('should not be valid if the field is empty', () => {
        ...
        expect(self.formController.cardholderName.$error.required).toEqual(
          true,
        );
      });
    ...
    describe('securityCode input', () => {
      ...
      it('should not be valid if it is less than 3 digits', () => {
        ...
        expect(self.formController.securityCode.$valid).toEqual(false);
        expect(
          self.formController.securityCode.$error.required,
        ).toBeUndefined();
