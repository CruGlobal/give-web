...
describe('userMatchModal', function () {
  ...
  describe('$onInit()', () => {
    beforeEach(() => {
      jest
        .spyOn($ctrl.verificationService, 'getContacts')
    ...
    describe("donorDetails registration-state='NEW'", () => {
      beforeEach(() => {
        jest
          .spyOn($ctrl.profileService, 'getDonorDetails')
          .mockReturnValue(Observable.of({ 'registration-state': 'NEW' }));
    ...
    describe("donorDetails registration-state='MATCHED'", () => {
      beforeEach(() => {
        jest
          .spyOn($ctrl.profileService, 'getDonorDetails')
          .mockReturnValue(Observable.of({ 'registration-state': 'MATCHED' }));
      });
  ...
  describe('loadContacts()', () => {
    ...
    it("initializes the component and proceeds to 'identity'", () => {
      const contacts = [
        { name: 'Charles Xavier', selected: false },
        { name: 'Bruce Bannr', selected: false },
      ];
      $ctrl.verificationService.getContacts.mockImplementation(() =>
    ...
    it("initializes the component and proceeds to 'activate'", () => {
      ...
      jest
        .spyOn($ctrl.verificationService, 'getQuestions')
        .mockReturnValue(
          Observable.of([{ key: 'a' }, { key: 'b' }, { key: 'c' }]),
        );
      $ctrl.verificationService.getContacts.mockImplementation(() =>
        Observable.of(contacts),
      );
    ...
    it('logs an error on failure', () => {
      $ctrl.verificationService.getContacts.mockReturnValue(
      ...
      expect($ctrl.loadingDonorDetailsError).toEqual(true);
      expect($ctrl.$log.error.logs[0]).toEqual([
        'Error loading verification contacts.',
        'another error',
      ]);
      expect($ctrl.setLoading).toHaveBeenCalledWith({ loading: false });
    });
  ...
  describe('postDonorMatch()', () => {
    ...
    it('loads contacts on donor match success', () => {
      jest
        .spyOn($ctrl.verificationService, 'postDonorMatches')
  ...
  describe('onSelectContact( contact )', () => {
    ...
    describe('valid contact', () => {
      it('selects the contact', () => {
        jest
          .spyOn($ctrl.verificationService, 'selectContact')
          .mockReturnValue(Observable.of({}));
        $ctrl.onSelectContact(true, { name: 'Batman' });

        expect($ctrl.setLoading).toHaveBeenCalledWith({ loading: true });
        expect($ctrl.verificationService.selectContact).toHaveBeenCalledWith({
          name: 'Batman',
        });
        expect($ctrl.changeMatchState).toHaveBeenCalledWith('activate');
      ...
      it('should log an error on failure', () => {
        ...
        expect($ctrl.$log.error.logs[0]).toEqual([
          'Error selecting verification contact.',
          'some error',
        ]);
        expect($ctrl.setLoading).toHaveBeenCalledWith({ loading: false });
      });
    ...
    describe('undefined', () => {
      it("selects 'that-is-not-me'", () => {
        jest
          .spyOn($ctrl.verificationService, 'thatIsNotMe')
          .mockReturnValue(Observable.of({}));
  ...
  describe('loadQuestions()', () => {
    ...
    it('should log an error on failure', () => {
      ...
      expect($ctrl.$log.error.logs[0]).toEqual([
        'Error loading verification questions.',
        'some error',
      ]);
      expect($ctrl.setLoading).toHaveBeenCalledWith({ loading: false });
    });
  ...
  describe('onQuestionAnswer()', () => {
    ...
    describe('no more questions', () => {
      ...
      it('proceeds to success on submitAnswers success', () => {
        jest
          .spyOn($ctrl.verificationService, 'submitAnswers')
          .mockReturnValue(Observable.of({}));
        ...
        expect($ctrl.verificationService.submitAnswers).toHaveBeenCalledWith([
          { key: 'a', answer: 'a' },
          {
            key: 'key',
            answer: 'answer',
