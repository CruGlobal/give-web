...
describe('session service', function () {
  ...
  describe('signOut()', () => {
    beforeEach(() => {
      jest
        .spyOn(sessionService.authClient, 'revokeAccessToken')
        .mockResolvedValue();
      jest
        .spyOn(sessionService.authClient, 'revokeRefreshToken')
        .mockResolvedValue();
      jest
        .spyOn(sessionService.authClient, 'signOut')
    ...
    describe('Successful cortex logout', () => {
      ...
      it('makes a DELETE request to Cortex & redirects to Okta to sign out', (done) => {
        sessionService.signOut(false).subscribe(() => {
          expect($window.location.href).toEqual(
            `${envService.read('oktaUrl')}/login/signout?fromURI=${envService.read('oktaReferrer')}/sign-out.html`,
          );
          done();
        }, done);
        $httpBackend.flush();
      ...
      it('should revoke all tokens & redirect to Okta to sign out', (done) => {
        sessionService.signOut(true).subscribe(() => {
          expect(
            sessionService.authClient.revokeAccessToken,
          ).toHaveBeenCalled();
          expect(
            sessionService.authClient.revokeRefreshToken,
          ).toHaveBeenCalled();
          expect($window.location.href).toEqual(
            `${envService.read('oktaUrl')}/login/signout?fromURI=${envService.read('oktaReferrer')}/sign-out.html`,
          );
      ...
      it('should add forcedUserToLogout session data', (done) => {
        sessionService.signOut(false).subscribe(() => {
          expect($window.sessionStorage.getItem(forcedUserToLogout)).toEqual(
            'true',
          );
          done();
        }, done);
  ...
  describe('signIn() & handleOktaRedirect()', () => {
    ...
    it('should return SignInEvent broadcast when not authenticated', (done) => {
      ...
      sessionService
        .signIn('lastPurchaseId-SignInEvent', loginHint)
        .subscribe();
      ...
      setTimeout(() => {
        ...
        expect(
          sessionService.authClient.token.getWithRedirect,
    ...
    it('should return SignInEvent broadcast when authenticated', (done) => {
      ...
      jest
        .spyOn(sessionService.authClient.tokenManager, 'getTokens')
    ...
    it('should trigger BodySignInEvent event', (done) => {
      ...
      window.document.body.addEventListener(
        BodySignInEvent,
        giveSignInSuccessCallback,
        { once: true },
      );

      sessionService.signIn().subscribe(() => {
        expect(giveSignInSuccessCallback).toHaveBeenCalledWith(
    ...
    it('should handle a failed login', (done) => {
      jest
        ...
        .mockResolvedValueOnce(false);
      jest
        .spyOn(sessionService.authClient, 'isLoginRedirect')
        .mockResolvedValueOnce(true);
      jest
        .spyOn(sessionService.authClient.token, 'parseFromUrl')
        .mockResolvedValue({});
    ...
    it('should ignore a non-login attempt', (done) => {
      ...
      sessionService.handleOktaRedirect().subscribe((data) => {
        expect(
          sessionService.authClient.token.parseFromUrl,
    ...
    describe('successful handleOktaRedirect()', () => {
      ...
      beforeEach(() => {
        isAuthenticatedSpy = jest.spyOn(
          sessionService.authClient,
          'isAuthenticated',
        );
        ...
        jest
          .spyOn(sessionService.authClient.tokenManager, 'setTokens')
      ...
      it('should remove locationSearchOnLogin when returning from Okta a successful login', (done) => {
        ...
        $window.sessionStorage.setItem(
          'locationSearchOnLogin',
          '?ga=111111&query=test&anotherQuery=00000',
        );
        ...
        setTimeout(() => {
          ...
          expect($location.search).toHaveBeenNthCalledWith(
            3,
            'anotherQuery',
            '00000',
          );
      ...
      it('should redirect to Okta from the login screen if the login has not yet happened', (done) => {
        ...
        sessionService.handleOktaRedirect().subscribe(() => {
          ...
          expect($window.sessionStorage.getItem(locationSearchOnLogin)).toEqual(
            '?ga=111111&query=test&anotherQuery=00000',
          );
          expect($window.sessionStorage.getItem(redirectLocation)).toEqual(
            null,
          );
          done();
        });
  ...
  describe('downgradeToGuest( skipEvent )', () => {
    ...
    describe("with 'IDENTIFIED' role", () => {
      ...
      it('make http request to cas/downgrade', (done) => {
        $httpBackend
          .expectPOST('https://give-stage2.cru.org/okta/downgrade', {})
          .respond(204, {});
  ...
  describe('updateCurrentProfile', () => {
    it('updates the first and last name if the cookie is defined', () => {
      ...
      $cookies.put(
        Sessions.profile,
        `.${btoa(angular.toJson(cruProfileCookie))}.`,
      );
      sessionService.updateCurrentProfile();
      expect(sessionService.session.first_name).toEqual(
        cruProfileCookie.first_name,
      );
      expect(sessionService.session.last_name).toEqual(
        cruProfileCookie.last_name,
      );
    });
  ...
  describe('CheckoutSavedData', () => {
    describe('updateCheckoutSavedData', () => {
      it('saves the checkout data to a cookie', () => {
        ...
        const dataStoredInLocalStorage = $cookies.get(
          checkoutSavedDataCookieName,
        );
