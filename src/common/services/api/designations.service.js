...
class DesignationsService {
    ...
    return Observable.from(
      ...
      return map(response.data.hits, (hit) => {
        ...
        return {
          ...
          replacementDesignationNumber:
            hit.replacement_designation_number || null,
          name: hit.title || hit.description || null,
          type: hit.designation_type || null,
          facet: hit.facet || null,
          startMonth: hit.start_date
            ? moment(hit.start_date).format('YYYY-MM-01')
            : null,
          ministry: hit.organization_id
            ? ministryIds[hit.organization_id]
            : null,
  ...
  productLookup(query, selectQuery) {
    const zoomObj = {
      ...
      choices:
        'definition:options:element:selector:choice[],definition:options:element:selector:choice:description,definition:options:element:selector:choice:selectaction',
      chosen:
        'definition:options:element:selector:chosen,definition:options:element:selector:chosen:description',
    };
    ...
    return httpRequest.map((data) => {
      if (!data.code)
        throw new Error('Product lookup response contains no code data');
      if (!data.offer)
        throw new Error('Product lookup response contains no offer data');
      if (!data.definition)
        throw new Error('Product lookup response contains no definition data');
      if (!data.choices)
        throw new Error('Product lookup response contains no choices data');
      if (!data.chosen)
        throw new Error('Product lookup response contains no chosen data');
      const choices = map(data.choices.concat(data.chosen), (choice) => {
        return {
          ...
          selectAction:
            (choice.selectaction && choice.selectaction.self.uri) ||
      ...
      .map((data) => {
        ...
        if (data.data['jcr:content']) {
          ...
          if (data.data['jcr:content'].suggestedAmounts) {
            angular.forEach(
              data.data['jcr:content'].suggestedAmounts,
              (v, k) => {
                if (toFinite(k) > 0 || k.includes('item')) {
                  suggestedAmounts.push({
                    ...
                    label: v.description,
                    order:
                      toFinite(k) > 0 ? toFinite(k) : Number(k.substring(4)),
                  });
                }
              },
            );
          ...
          if (
            data.data['jcr:content'].defaultCampaign &&
            !itemConfig.CAMPAIGN_CODE
          ) {
            itemConfig['default-campaign-code'] =
              data.data['jcr:content'].defaultCampaign;
          }
          // Copy jcr:title
  ...
  facebookPixel(code) {
    return Observable.from(this.designationData(code)).map((data) => {
  ...
  ministriesList(pagePath) {
    const path =
      pagePath + '/jcr:content/content-parsys/designation_search_r.json';
    const url = `${this.$location.protocol()}://${this.$location.host()}/${path}`;
    return Observable.from(this.$http.get(url)).map((response) => {
