...
describe('order service', () => {
  ...
  const paymentMethodForms = cloneDeep(
    cartResponse._order[0]._paymentmethodinfo[0]._element,
  );
  ...
  const purchaseFormResponseZoomMapped = {
    enhancedpurchaseform:
      purchaseFormResponse._order[0]._enhancedpurchaseform[0],
  ...
  describe('getDonorDetails', () => {
    it('should load the donorDetails', (done) => {
      const donorDetailsResponseZoomMapped = {
        ...
        orderEmailForm:
          donorDetailsResponse._order[0]._emailinfo[0]._orderemailform[0],
        rawData: donorDetailsResponse,
      };

      //
      const expectedDonorDetails = omit(
        donorDetailsResponseZoomMapped.donorDetails,
        'mailing-address',
      );
      expectedDonorDetails.mailingAddress = formatAddressForTemplate(
        donorDetailsResponseZoomMapped.donorDetails['mailing-address'].address,
      );
      expectedDonorDetails.email = donorDetailsResponseZoomMapped.email.email;
      expectedDonorDetails.emailFormUri =
        donorDetailsResponseZoomMapped.orderEmailForm.links[0].uri;

      self.$httpBackend
        .expectGET(
          'https://give-stage2.cru.org/cortex/carts/crugive/default?zoom=order:donordetails,order:emailinfo:email,order:emailinfo:orderemailform',
        )
        .respond(200, donorDetailsResponse);
      self.orderService.getDonorDetails().subscribe((data) => {
    ...
    it('should set the mailingAddress country to US if undefined', (done) => {
      donorDetailsResponse._order[0]._donordetails[0]['mailing-address'][
        'country-name'
      ] = '';
      self.$httpBackend
        .expectGET(
  ...
  describe('addEmail', () => {
    it('should send a request to save the email', (done) => {
      self.$httpBackend
        .expectPOST('https://give-stage2.cru.org/cortex/emails/crugive', {
          email: 'someemail@somedomain.com',
        })
        .respond(200, 'somedata');
      self.orderService
        .addEmail('someemail@somedomain.com', '/emails/crugive')
        .subscribe((data) => {
          expect(data).toEqual('somedata');
          done();
        });
  ...
  describe('getPaymentMethodForms', () => {
    function setupRequest() {
      self.$httpBackend
        .expectGET(
          'https://give-stage2.cru.org/cortex/carts/crugive/default?zoom=order:paymentmethodinfo:element,order:paymentmethodinfo:element:paymentinstrumentform',
        )
        .respond(200, cartResponse);
    }

    function initiateRequest(done) {
      self.orderService.getPaymentMethodForms().subscribe((data) => {
    ...
    it('should use the cached response if called a second time', (done) => {
      ...
      self.orderService.getPaymentMethodForms().subscribe((data) => {
        expect(data).toEqual(cartResponseZoomMapped);
        self.orderService.getPaymentMethodForms().subscribe((data2) => {
    ...
    it('should log a warning if there is no create payment instrument link', (done) => {
      ...
      self.orderService.getPaymentMethodForms().subscribe((data) => {
        expect(self.$log.warn.logs[0]).toContain(
          'Payment form request contains empty link',
        );
        done();
      });
      self.$httpBackend.flush();
  ...
  describe('addBankAccountPayment', () => {
    it('should send a request to save the bank account payment info', (done) => {
      ...
      self.orderService.addBankAccountPayment(paymentInfo).subscribe((data) => {
  ...
  describe('addCreditCardPayment', () => {
    ...
    it('should send a request to save the credit card payment info with no billing address', (done) => {
      ...
      self.orderService.addCreditCardPayment(paymentInfo).subscribe((data) => {
      ...
      expect(self.orderService.storeCardSecurityCode).toHaveBeenCalledWith(
        '456',
        'new cc uri',
      );
    });
  ...
  describe('addPaymentMethod', () => {
    it('should save a new bank account payment method', (done) => {
      jest
        .spyOn(self.orderService, 'addBankAccountPayment')
      ...
      expect(self.orderService.addBankAccountPayment).toHaveBeenCalledWith(
        paymentInfo,
        false,
      );
    });
    ...
    it('should tell the system to save the new bank account on the donor profile', (done) => {
      jest
        .spyOn(self.sessionService, 'getRole')
    ...
    it('should save a new credit card payment method', (done) => {
      jest
        .spyOn(self.orderService, 'addCreditCardPayment')
        .mockReturnValue(
          Observable.of({ self: { uri: 'new payment method uri' } }),
        );
      const paymentInfo = {
        address: {
          'country-name': 'US',
      ...
      expect(self.orderService.addCreditCardPayment).toHaveBeenCalledWith(
        paymentInfo,
        false,
      );
    });
        ...
        .subscribe(
          ...
          (error) => {
            expect(error).toEqual(
              'Error adding payment method. The data passed to orderService.addPaymentMethod did not contain bankAccount or creditCard data',
            );
          },
        );
    });
  ...
  describe('updatePaymentMethod', () => {
    ...
    beforeEach(() => {
      jest
        ...
        .mockImplementation(() => {});
      jest
        .spyOn(self.orderService, 'selectPaymentMethod')
        .mockReturnValue(Observable.of('placeholder'));

      runTestWith = (paymentInfo, expectedRequestData, expectedCvv) => {
        self.$httpBackend
          .expectGET(
            'https://give-stage2.cru.org/cortex/carts/crugive/default?zoom=order:paymentinstrumentselector:chosen,order:paymentinstrumentselector:chosen:description',
          )
          .respond(200, {
            _order: [
              {
        ...
        self.$httpBackend
          .expectPUT(
            'https://give-stage2.cru.org/cortex/paymentinstruments/orders/crugive/<order id>=/orderpaymentinstrument/<payment id>=',
            expectedRequestData,
          )
          .respond(200, {});
          ...
          .updatePaymentMethod(
            {
              ...
              self: { uri: 'existing payment method uri' },
            },
            { creditCard: paymentInfo },
          )
          .subscribe();

        expect(self.orderService.selectPaymentMethod).toHaveBeenCalledWith(
          '<select uri>',
        );
    ...
    it('should update a credit card that has already been selected', () => {
      ...
      const expectedPaymentMethodUri =
        '/paymentinstruments/orders/crugive/<order id>=/orderpaymentinstrument/<payment id>=';
      const paymentMethod = {
        selectAction: '<select uri>',
        self: {
      ...
      self.$httpBackend
        .expectPUT(
          `https://give-stage2.cru.org/cortex${expectedPaymentMethodUri}`,
          expectedRequestData,
        )
        .respond(200, {});

      self.orderService
        .updatePaymentMethod(paymentMethod, { creditCard: paymentInfo })
        .subscribe();
  ...
  describe('getExistingPaymentMethods', () => {
    ...
    beforeEach(() => {
      clonedPaymentMethodSelectorResponse = angular.copy(
        paymentMethodSelectorResponse,
      );
      expectedResponse = [
        {
          self: {
          ...
          'encrypted-account-number':
            '0d640924-8399-48b5-851e-808e308c2a8a:GT1pVOxr5KenKbRrvYUpnw',
          'routing-number': '121042882',
          'saved-on-profile': true,
          chosen: true,
          selectAction:
            '/paymentinstruments/orders/crugive/mnrwmntdmjrgkljvgi4gmljugrstcllbmjqtsllegq2winbvgbrdamrzgm=/paymentinstrumentselector/qk2wgylsoqww64temvzc22loon2he5lnmvxhjwjegazdgmlgha3gmllbha4wmljugi3dmljzguygmllemrsgimtegrqwmnlfmsvgs3ttorzhk3lfnz2nsjbrmftgmojugmzs2mrqha4s2nbqgqzs2ojwgztc2m3emvrtamrtha4tszlc=',
        },
        {
          self: {
            type: 'paymentinstruments.payment-instrument',
          ...
          selectAction:
            '/paymentinstruments/orders/crugive/mnrwmntdmjrgkljvgi4gmljugrstcllbmjqtsllegq2winbvgbrdamrzgm=/paymentinstrumentselector/qkzwg5ltorxw2zlsfvuw443uoj2w2zlootmsinjzgzqtcntemmwtin3egqwtintcmiwwenlfmywtmylgme2dazbtg43tmy5knfxhg5dsovwwk3tu3esdknzzg5sdizjxfu3dinlbfu2dendcfu4tiyjtfu2giyjyge3wcmbwmfsgc=',
        },
        {
          self: {
            type: 'paymentinstruments.payment-instrument',
          ...
          selectAction:
            '/paymentinstruments/orders/crugive/mnrwmntdmjrgkljvgi4gmljugrstcllbmjqtsllegq2winbvgbrdamrzgm=/paymentinstrumentselector/qkzwg5ltorxw2zlsfvuw443uoj2w2zlootmsim3cmy2wcmzzmmwtmylfgewtin3fgawwcnzzmiwwgndggmzgmmrqgyzwcyvknfxhg5dsovwwk3tu3esdinbwge4tonlgfu2wenbzfu2doytbfvqtimjqfu2gmnbvha2dmnrrgi2wg=',
        },
      ];
    });
    ...
    it("should load a user's existing payment methods", (done) => {
      ...
      self.orderService.getExistingPaymentMethods().subscribe((data) => {
    ...
    it("should load a user's existing payment methods even if there is no chosen one", (done) => {
      ...
      clonedPaymentMethodSelectorResponse._order[0]._paymentinstrumentselector[0]._choice.push(
        clonedPaymentMethodSelectorResponse._order[0]
          ._paymentinstrumentselector[0]._chosen[0],
      );
      delete clonedPaymentMethodSelectorResponse._order[0]
        ._paymentinstrumentselector[0]._chosen;
    ...
    it("should load a user's existing payment methods even if there is no choice element and only a chosen one", (done) => {
      ...
      delete clonedPaymentMethodSelectorResponse._order[0]
        ._paymentinstrumentselector[0]._choice;
    ...
    it('should format payment addresses while loading existing payment methods', (done) => {
      clonedPaymentMethodSelectorResponse._order[0]._paymentinstrumentselector[0]._choice[0]._description[0][
        'payment-instrument-identification-attributes'
      ]['extended-address'] = 'Apt B';
      expectedResponse[2].address = {
        country: 'US',
        streetAddress: '1234 First Street',
  ...
  describe('selectPaymentMethod', () => {
    it('should post the URI of the selected payment method for cortex to select it', (done) => {
      ...
      self.orderService
        .selectPaymentMethod(
          '/paymentmethods/crugive/giydembug4=/selector/orders/crugive/mm2tsnrrg5qtqljshfsteljuhe2dellcgrrweljugftdgndcg4zweztbmq=',
        )
        .subscribe((data) => {
          expect(data).toEqual('success');
          done();
  ...
  describe('getCurrentPayment', () => {
    it('should retrieve the current payment details', (done) => {
      ...
      self.orderService.getCurrentPayment().subscribe((data) => {
        expect(data).toEqual(expectedResponse);
        done();
      });
    ...
    it('should retrieve the current payment details with a billing address', (done) => {
      ...
      const expectedPaymentInfo = angular.copy(
        paymentMethodCreditCardResponse._order[0]._paymentinstrumentselector[0]
          ._chosen[0]._description[0],
      );
      ...
      expectedPaymentInfo['payment-instrument-identification-attributes'] =
        undefined;
  ...
  describe('getPurchaseForms', () => {
    it('should send a request to get the payment form links', (done) => {
      self.$httpBackend
        .expectGET(
          'https://give-stage2.cru.org/cortex/carts/crugive/default?zoom=order:enhancedpurchaseform',
        )
        .respond(200, purchaseFormResponse);
      self.orderService.getPurchaseForm().subscribe((data) => {
  ...
  describe('checkErrors', () => {
    it('should send a request to get the payment form links', (done) => {
      self.$httpBackend
        .expectGET(
          'https://give-stage2.cru.org/cortex/carts/crugive/default?zoom=order',
        )
        .respond(200, needInfoResponse);
      self.orderService.checkErrors().subscribe((data) => {
        expect(data).toEqual([
          'need.email',
  ...
  describe('submit', () => {
    ...
    beforeEach(() => {
      ...
      getPurchaseFormMock = jest
        .spyOn(self.orderService, 'getPurchaseForm')
        .mockReturnValue(Observable.of(purchaseFormResponseZoomMapped));
    });
    ...
    it('should send a request to finalize the purchase', (done) => {
      self.$httpBackend
        .expectPOST(
          ...
          {
            'cover-cc-fees': false,
            'radio-call-letters': null,
            'tsys-device': '',
            'recaptcha-token': null,
            'recaptcha-action': null,
          },
        )
        .respond(200, purchaseResponse);

      self.orderService.submit().subscribe((data) => {
    ...
    it('should send a request to finalize the purchase with a CVV and cardBin', (done) => {
      ...
      self.orderService.submit('123', '411111').subscribe((data) => {
        expect(data).toEqual(purchaseResponse);
        done();
      });
    ...
    it('should clear the recaptcha data from session storage', (done) => {
      ...
      self.orderService.submit().subscribe((data) => {
        expect(self.$window.sessionStorage.getItem('recaptchaToken')).toEqual(
          null,
        );
        expect(self.$window.sessionStorage.getItem('recaptchaAction')).toEqual(
          null,
        );
        done();
      });
  ...
  describe('storeCardSecurityCode', () => {
    it('should save the cvv to session storage', () => {
      ...
      expect(self.$window.sessionStorage.getItem('storedCvvs')).toEqual(
        '{"new id":"123"}',
      );
    });
  ...
  describe('retrieveCardSecurityCodes', () => {
    it('should return the stored cvvs', () => {
      self.$window.sessionStorage.setItem(
        'storedCvvs',
        '{"/paymentmethods/crugive/giydsnjqgi=":"123","/paymentmethods/crugive/giydsnjqgy=":"321"}',
      );
  ...
  describe('cardBin', () => {
    ...
    describe('retrieveCardBins', () => {
      it('should return the stored cardBins', () => {
        self.$window.sessionStorage.setItem(
          'storedBins',
          '{"/paymentmethods/crugive/giydsnjqgi=":"411111","/paymentmethods/crugive/giydsnjqgy=":"411111"}',
        );
    ...
    describe('clearCardBins', () => {
      it('should clear the stored the card bin', () => {
        self.$window.sessionStorage.setItem('cardBin', '411111');
        self.$window.sessionStorage.setItem('storedBins', {
          'some uri': '411111',
        });
  ...
  describe('storeLastPurchaseLink', () => {
    it('should save the link to the completed purchase', () => {
      ...
      expect(self.$window.sessionStorage.getItem('lastPurchaseLink')).toEqual(
        '/purchases/crugive/giydanbt=',
      );
    });
  });
  ...
  describe('retrieveLastPurchaseLink', () => {
    it('should save the link to the completed purchase', () => {
      self.$window.sessionStorage.setItem(
        'lastPurchaseLink',
        '/purchases/crugive/hiydanbt=',
      );

      expect(self.orderService.retrieveLastPurchaseLink()).toEqual(
        '/purchases/crugive/hiydanbt=',
      );
    });
  });
  ...
  describe('storeRadioStationData', () => {
    it('should save the choice of radio station', () => {
      ...
      expect(self.$window.sessionStorage.getItem('radioStationName')).toEqual(
        'Radio Station',
      );
      expect(
        self.$window.sessionStorage.getItem('radioStationCallLetters'),
      ).toEqual('WXYZ');
    });
  });
  ...
  describe('retrieveRadioStationName', () => {
    it('should remember the choice of radio station name', () => {
      ...
      expect(self.orderService.retrieveRadioStationName()).toEqual(
        'Radio Station',
      );
    });
  });
  ...
  describe('retrieveRadioStationCallLetters', () => {
    it('should remember the choice of radio station call letters', () => {
      self.$window.sessionStorage.setItem('radioStationCallLetters', 'WXYZ');
      expect(self.orderService.retrieveRadioStationCallLetters()).toEqual(
        'WXYZ',
      );
    });
  });
  ...
  describe('spouseEditableForOrder', () => {
    it('should not be editable for staff', () => {
      expect(self.orderService.spouseEditableForOrder({ staff: true })).toEqual(
        false,
      );
    });
    ...
    it('should be editable during a new order when the spouse name fields are empty', () => {
      expect(
        self.orderService.spouseEditableForOrder({
          'spouse-name': {},
        }),
      ).toEqual(true);

      expect(
        self.$window.localStorage.getItem('startedOrderWithoutSpouse'),
      ).toEqual('true');
    });
    ...
    it('should store the current order uri during a new order', () => {
      ...
      expect(self.$window.localStorage.getItem('currentOrder')).toEqual(
        'order id',
      );
    });
  ...
  describe('submitOrder', () => {
    ...
    describe('submit single order', () => {
      ...
      afterEach(() => {
        expect(mockController.onSubmittingOrder).toHaveBeenCalledWith({
          value: true,
        });
        expect(mockController.onSubmittingOrder).toHaveBeenCalledWith({
          value: false,
      ...
      it('should submit the order normally if paying with a bank account', (done) => {
        ...
        self.orderService.submitOrder(mockController).subscribe(() => {
          ...
          expect(
            self.orderService.saveDonorDataForRegistration,
          ).toHaveBeenCalledWith(mockDonorDetails);

          expect(mockController.$scope.$emit).toHaveBeenCalledWith(
            cartUpdatedEvent,
          );
          done();
        }, done);
      ...
      it('should handle an error submitting an order with a bank account', (done) => {
        self.orderService.submit.mockImplementation(() =>
          Observable.throw({ data: 'error saving bank account' }),
        );
        ...
        self.orderService.submitOrder(mockController).subscribe(
          ...
          () => {
            ...
            expect(self.$log.error.logs[0]).toEqual([
              ...
              { data: 'error saving bank account' },
            ]);
      ...
      it('should submit the order with a CVV and cardBin if paying with a credit card', (done) => {
        ...
        self.orderService.submitOrder(mockController).subscribe(() => {
          expect(self.orderService.submit).toHaveBeenCalledWith(
            '1234',
            '411111',
          );
          expect(self.orderService.clearCardSecurityCodes).toHaveBeenCalled();
      ...
      it('should submit the order without a CVV and cardBin if paying with an existing credit card or the cvv in session storage is missing', (done) => {
        ...
        self.orderService.retrieveCardBin = jest
          .fn()
          .mockReturnValue(undefined);
        mockController.coverFeeDecision = true;
        self.orderService.submitOrder(mockController).subscribe(() => {
      ...
      it('should handle an error submitting an order with a credit card', (done) => {
        self.orderService.submit.mockImplementation(() =>
          Observable.throw({
            data: 'CardErrorException: Invalid Card Number: some details',
          }),
        );
        mockController.creditCardPaymentDetails = {};
        self.orderService.retrieveCardSecurityCode = jest
      ...
      it('should mask the security code on a credit card error', (done) => {
        self.orderService.submit.mockReturnValue(
          Observable.throw({
            data: 'some error',
            config: { data: { 'security-code': '1234' } },
          }),
        );
        self.orderService.retrieveCardSecurityCode = jest
          .fn()
        ...
        self.orderService.submitOrder(mockController).subscribe(
          ...
          () => {
            ...
            expect(self.$log.error.logs[0]).toEqual([
              ...
              {
                data: 'some error',
                config: { data: { 'security-code': 'XXXX' } },
              },
            ]);
            done();
          },
      ...
      it('should throw an error if neither bank account or credit card details are loaded', (done) => {
        self.orderService.submitOrder(mockController).subscribe(
          ...
          () => {
            ...
            expect(self.$log.error.logs[0]).toEqual([
              'Error submitting purchase:',
              { data: 'Current payment type is unknown' },
            ]);
            expect(mockController.submissionError).toEqual(
              'Current payment type is unknown',
            );
            expect(self.$window.scrollTo).toHaveBeenCalledWith(0, 0);
            done();
          },
  ...
  describe('logToDatadogRum', () => {
    ...
    it('should log a checkout error', () => {
      ...
      expect(self.orderService.datadogRum.addError).toHaveBeenCalledWith(
        new Error(`Error submitting purchase: ${JSON.stringify(error)}`),
        { context: 'Checkout Submission', errorCode: error.status },
      );
    });
