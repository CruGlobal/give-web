...
describe('cart service', () => {
  ...
  describe('get', () => {
    beforeEach(() => {
      ...
      jest
        ...
        .mockImplementation(() => {});
      jest
        .spyOn(self.cartService.$location, 'host')
        .mockReturnValue('give.cru.org');
      jest
        .spyOn(self.cartService.commonService, 'getNextDrawDate')
        .mockReturnValue(Observable.of('2016-10-01'));
    ...
    it('should handle an empty response', () => {
      self.$httpBackend
        .expectGET(
          'https://give-stage2.cru.org/cortex/carts/crugive/default' +
      ...
      self.cartService.get().subscribe((data) => {
        expect(data).toEqual({});
      });
    ...
    it('should handle a response with no line items', () => {
      ...
      self.cartService.get().subscribe((data) => {
        expect(data).toEqual({});
        expect(self.cartService.$cookies.remove).toHaveBeenCalledWith(
          'giveCartItemCount',
          expect.any(Object),
        );
      });
    ...
    it('should get cart, parse response, and show most recent items first', () => {
      ...
      self.cartService.get().subscribe((data) => {
        // verify response
        expect(
          self.cartService.commonService.getNextDrawDate,
        ).toHaveBeenCalled();
        ...
        expect(data.items[1].giftStartDate.toString()).toEqual(
          moment('2016-10-09').toString(),
        );
        ...
        expect(self.cartService.$cookies.put).toHaveBeenCalledWith(
          'giveCartItemCount',
          3,
          expect.any(Object),
        );
    ...
    it('should not set cart count cookie on other domains', () => {
      ...
      self.cartService.get().subscribe(() => {
  ...
  describe('handleCartResponse', () => {
    const zoom = {
      lineItems:
        'lineitems:element[],lineitems:element:availability,lineitems:element:item,lineitems:element:item:code,lineitems:element:item:definition,lineitems:element:rate,lineitems:element:total,lineitems:element:itemfields',
    ...
    beforeEach(() => {
      ...
      jest
        ...
        .mockReturnValue('give.cru.org');
      advanceTo(moment('2016-09-01').toDate()); // Make sure current date is before next draw date
      transformedCartResponse =
        self.cartService.hateoasHelperService.mapZoomElements(
          cartResponse,
          zoom,
        );
      ...
      transformedCartResponse.rateTotals[0].cost['display-with-fees'] =
        '$52.23';
    });

    it('should get cart, parse response, and show most recent items first', () => {
      const data = self.cartService.handleCartResponse(
        transformedCartResponse,
        '2016-10-01',
      );
      // verify response
    ...
    it('should handle total correctly if the order changes from the API', () => {
      transformedCartResponse.rateTotals.unshift(
  ...
  describe('addItem', () => {
    beforeEach(() => {
      jest
        .spyOn(self.cartService.sessionService, 'getRole')
    ...
    it('should add an item', () => {
      ...
      self.cartService
        .addItem('items/crugive/<some id>', { amount: 50 })
        .subscribe();
      self.$httpBackend.flush();
    });
    ...
    describe('as a public user', () => {
      ...
      describe('with existing cart', () => {
        beforeEach(() => {
          jest
            .spyOn(self.cartService, 'getTotalQuantity')
      ...
      describe('with empty cart', () => {
        beforeEach(() => {
          jest
            .spyOn(self.cartService, 'getTotalQuantity')
            .mockReturnValue(Observable.of(0));
          jest
            .spyOn(self.cartService.sessionService, 'signOut')
        ...
        it('should delete cookies and addItem to cart when not authenticated', () => {
          jest
            .spyOn(self.cartService.sessionService, 'oktaIsUserAuthenticated')
            .mockReturnValue(Observable.from([false]));
          jest.spyOn(self.cartService, '_addItem');
          self.cartService
            .addItem('items/crugive/<some id>', { amount: 50 })
            .subscribe();
          expect(
            self.cartService.sessionService.oktaIsUserAuthenticated,
          ).toHaveBeenCalled();
          expect(
            self.cartService.sessionService.signOut,
        ...
        it('should force user to sign out of Okta prior to adding item to cart', () => {
          jest
            .spyOn(self.cartService.sessionService, 'oktaIsUserAuthenticated')
            .mockReturnValue(Observable.from([true]));
          jest.spyOn(self.cartService, '_addItem');
          self.cartService
            .addItem('items/crugive/<some id>', { amount: 50 })
            .subscribe();
  ...
  describe('editItem', () => {
    it('should delete the old item and add the new one', () => {
      jest
        .spyOn(self.cartService, 'deleteItem')
        .mockReturnValue(Observable.of({ data: null }));
      jest
        .spyOn(self.cartService, 'addItem')
        .mockReturnValue(Observable.of({ data: null }));
      self.cartService
        .editItem('<some old id>', '<new id>', { code: '<some code>' })
        .subscribe();

      expect(self.cartService.deleteItem).toHaveBeenCalledWith('<some old id>');
      expect(self.cartService.addItem).toHaveBeenCalledWith(
        '<new id>',
        { code: '<some code>' },
        true,
      );
    ...
    it('should give me data from the add item', (done) => {
      ...
      jest
        .spyOn(self.cartService, 'addItem')
        .mockReturnValue(Observable.of(addData));
      self.cartService
        .editItem('<some old id>', '<new id>', { code: '<some code>' })
        .subscribe((data) => {
  ...
  describe('deleteItem', () => {
    it('should delete an item', () => {
      ...
      self.cartService
        .deleteItem('itemfieldslineitem/items/crugive/<some id>')
        .subscribe();
      self.$httpBackend.flush();
    });
  });
  ...
  describe('bulkAdd', () => {
    ...
    it('should throw an error if there are no designations found from lookup', () => {
      self.cartService.designationsService.bulkLookup.mockReturnValue(
        Observable.of({ links: [] }),
      );
      self.cartService.bulkAdd([{ designationNumber: '0123456' }]).subscribe(
    ...
    it('should combine the configured designations with their product uri and call addItemAndReplaceExisting for each designation', () => {
      self.cartService.designationsService.bulkLookup.mockReturnValue(
        Observable.of({ links: [{ uri: 'uri1' }, { uri: 'uri2' }] }),
      );
      jest
        .spyOn(self.cartService, 'addItemAndReplaceExisting')
        .mockImplementation((cart, uri, configuredDesignation) =>
          Observable.of({ configuredDesignation: configuredDesignation }),
        );
      const outputValues = [];
      self.cartService
        .bulkAdd([
        ...
        .subscribe(
          ...
          () => {
            expect(
              self.cartService.addItemAndReplaceExisting,
            ).toHaveBeenCalledWith(expect.any(Observable), 'carts/uri1/form', {
              designationNumber: '0123456',
              uri: 'carts/uri1/form',
            });
            expect(
              self.cartService.addItemAndReplaceExisting,
            ).toHaveBeenCalledWith(expect.any(Observable), 'carts/uri2/form', {
              designationNumber: '1234567',
              uri: 'carts/uri2/form',
            });
    ...
    it('should provide cart as an observable that only performs a cart request once', () => {
      ...
      self.cartService.bulkAdd([{ designationNumber: '0123456' }]).subscribe(
        ...
        () => {
          jest
            .spyOn(self.cartService, 'get')
            .mockReturnValue(Observable.empty());
          const cartObservable =
            self.cartService.addItemAndReplaceExisting.mock.calls[
              self.cartService.addItemAndReplaceExisting.mock.calls.length - 1
            ][0];
  ...
  describe('addItemAndReplaceExisting', () => {
    ...
    beforeEach(() => {
      jest
        ...
        .mockReturnValue(Observable.of({}));
      jest
        .spyOn(self.cartService, 'editItem')
        .mockReturnValue(Observable.of({}));
    });

    it('should add items to cart if there are no conflicts', () => {
      self.cartService
        .addItemAndReplaceExisting(null, 'uri1', {
          designationNumber: '0123456',
          amount: 51,
          uri: 'uri1',
        })
        .subscribe(
          (response) => {
            expect(self.cartService.addItem).toHaveBeenCalledWith('uri1', {
              amount: 51,
            });
    ...
    it('should catch a conflict in the cart and replace that item', () => {
      self.cartService.addItem.mockReturnValue(
        Observable.throw({ status: 409 }),
      );
      self.cartService
        .addItemAndReplaceExisting(cartObservable, 'uri1', {
          designationNumber: '0123456',
          amount: 51,
          uri: 'uri1',
        })
        .subscribe(
          (response) => {
            ...
            expect(self.cartService.editItem).toHaveBeenCalledWith(
              'oldUri',
              'uri1',
              { amount: 51 },
            );
  ...
  describe('buildCartUrl', () => {
    ...
    it('should build a url with query parameters', () => {
      ...
      self.cartService.$window.location.href +=
        self.cartService.$window.location.search;
