...
class HateoasHelper {
  ...
  mapZoomElements (data, zoom) {
    const dataObj = mapValues(zoom, (zoomString) => {
      ...
      const element = this.getElement(data, this.stripArrayIndicators(zoomString).split(':'), this.hasArrayIndicators(zoomString))
      if (!element) {
        return element
  ...
  mapChildZoomElements (element, parentZoomString, childZoomStrings) {
    ...
    const commonPrefixRegex = new RegExp('^' + escapeRegExp(this.stripArrayIndicators(parentZoomString)) + ':')
