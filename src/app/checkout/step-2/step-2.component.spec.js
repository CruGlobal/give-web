  ...
  describe('step 2', () => {
    ...
    describe('handleExistingPaymentLoading', () => {
      ...
      it('should set flags for the view if requesting payment methods fails', () => {
        expect(self.controller.loadingPaymentMethods).toEqual(true);
        self.controller.handleExistingPaymentLoading(
          false,
          undefined,
          'some error',
        );
        ...
        expect(self.controller.$log.warn.logs[0]).toEqual([
          'Error loading existing payment methods',
          'some error',
        ]);
        expect(self.controller.loadingExistingPaymentError).toEqual(
          'some error',
        );
      });
    });
    ...
    describe('submit', () => {
      it('should change form state', () => {
        ...
        ).toHaveBeenCalledWith(self.controller.defaultPaymentType, 'payment');
      });
    });
    ...
    describe('handlePaymentChange', () => {
      it('should change default payment type with bank account', () => {
        ...
        expect(
          self.controller.brandedAnalyticsFactory.savePaymentType,
        ).toHaveBeenCalledWith('checking', false);
      });
      ...
      it('should change default payment type with credit card', () => {
        ...
        expect(
          self.controller.brandedAnalyticsFactory.savePaymentType,
        ).toHaveBeenCalledWith('visa', true);
      });
    ...
    describe('onPaymentFormStateChange', () => {
      ...
      it('should save payment data when in the loading state with a payload', () => {
        ...
        self.controller.onPaymentFormStateChange({
          state: 'loading',
          payload: { bankAccount: { 'account-type': 'Checking' } },
        });

        expect(
          self.controller.orderService.addPaymentMethod,
        ).toHaveBeenCalledWith({ bankAccount: { 'account-type': 'Checking' } });
        expect(self.controller.changeStep).toHaveBeenCalledWith({
          newStep: 'review',
        });
        expect(self.controller.onStateChange).toHaveBeenCalledWith({
          state: 'submitted',
        });
        expect(self.controller.$onInit).toHaveBeenCalled();
      ...
      it('should save payment data and not change step when in the loading state with a payload and stayOnStep is true', () => {
        ...
        expect(
          self.controller.orderService.addPaymentMethod,
        ).toHaveBeenCalledWith({ bankAccount: {} });
      ...
      it('should handle an error saving payment data', () => {
        ...
        jest
          .spyOn(self.controller.orderService, 'addPaymentMethod')
          .mockReturnValue(Observable.throw({ data: 'some error' }));
        self.controller.onPaymentFormStateChange({
          state: 'loading',
          payload: { bankAccount: {} },
        });
      ...
      it('should update payment data', () => {
        jest
          .spyOn(self.controller.orderService, 'updatePaymentMethod')
          .mockReturnValue(Observable.of(''));
        self.controller.onPaymentFormStateChange({
          state: 'loading',
          payload: { creditCard: {} },
          update: true,
          paymentMethodToUpdate: 'selected payment method',
        });
        ...
        expect(
          self.controller.orderService.updatePaymentMethod,
        ).toHaveBeenCalledWith('selected payment method', { creditCard: {} });
      });
    });
    ...
    describe('isContinueDisabled', () => {
      ...
      it('should return true while the payment form is encrypting or loading', () => {
        ...
        jest
          .spyOn(self.controller.orderService, 'addPaymentMethod')
          .mockReturnValue(Observable.from(deferred.promise));
