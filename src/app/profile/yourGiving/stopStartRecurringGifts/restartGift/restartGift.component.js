...
import donationsService, {RecurringGiftsType} from 'common/services/api/donations.service';
...
class RestartGiftController {
  ...
  loadGiftsAndRecipients() {
    this.setLoading( {loading: true} );
    Observable.forkJoin( [
      this.donationsService.getRecurringGifts( RecurringGiftsType.suspended, true ),
      this.donationsService.getSuggestedRecipients()
    ] ).subscribe( ( [suspendedGifts, suggestedRecipients] ) => {
      this.suspendedGifts = suspendedGifts || [];
      this.suggestedRecipients = map( suggestedRecipients || [], recipient => (new RecurringGiftModel( recipient )).setDefaults() );
  ...
  next( selected, configured, paymentMethod ) {
    ...
    switch ( this.step ) {
      case 'suspended':
        if ( angular.isDefined( selected ) ) this.selectedGifts.suspended = selected;
        if ( this.includeSuggestedRecipients ) {
          this.step = 'suggested';
        } else if ( !this.selectedGifts.suspended.length && !this.selectedGifts.suggested.length ) {
          this.step = 'search';
        } else {
          this.configureGifts();
        }
        break;
      case 'suggested':
        if ( angular.isDefined( selected ) ) this.selectedGifts.suggested = selected;
        if ( !this.selectedGifts.suspended.length && !this.selectedGifts.suggested.length ) {
          this.step = 'search';
        } else {
          this.configureGifts();
        }
      ...
      case 'search':
        if ( angular.isDefined( selected ) ) {
          ...
          this.selectedGifts.search = map( selected, recipient => (new RecurringGiftModel( {
  ...
  configureGifts() {
    this.gifts = map( flatten( values( this.selectedGifts ) ), gift => gift.clone() );
    this.step = 'configure';
  }
