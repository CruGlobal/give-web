...
describe('your giving', function () {
  ...
  describe('$onDestroy()', () => {
    it('cleans up the component', () => {
      ...
      expect($ctrl.$location.search).toHaveBeenCalledTimes(
        keys(queryParams).length,
      );
    });
  });
  ...
  describe('loadProfile()', () => {
    ...
    it('successfully loads profile', () => {
      jest
        .spyOn($ctrl.profileService, 'getGivingProfile')
  ...
  describe('setGivingView( name )', () => {
    it('sets view based on url query param', () => {
      jest
        ...
        .mockReturnValue({ [queryParams.view]: 'historical' });

      $ctrl.setGivingView();

      expect($ctrl.$location.search).toHaveBeenCalled();
      expect($ctrl.view).toEqual('historical');
      expect($ctrl.$location.search).toHaveBeenCalledWith(
        queryParams.view,
        'historical',
      );
    });
  ...
  describe('openEditRecurringGiftsModal', () => {
    it('should open the modal', () => {
      ...
      jest
        ...
        .mockReturnValue({ result: { then: thenSpy } });
      $ctrl.openEditRecurringGiftsModal();

      expect($ctrl.$uibModal.open).toHaveBeenCalledWith(
        expect.objectContaining({ component: 'editRecurringGiftsModal' }),
      );
    ...
    it('should call analytics event on dismiss', () => {
      ...
      expect($ctrl.analyticsFactory.track).toHaveBeenCalledWith(
        'ga-edit-recurring-exit',
      );
    });
  });
  ...
  describe('openGiveOneTimeGiftModal', () => {
    it('should open the modal', () => {
      ...
      expect($ctrl.$uibModal.open).toHaveBeenCalledWith(
        expect.objectContaining({ component: 'giveOneTimeGiftModal' }),
      );
    });
  });
  ...
  describe('openStopStartRecurringGiftsModal', () => {
    it('should open the modal', () => {
      ...
      expect($ctrl.$uibModal.open).toHaveBeenCalledWith(
        expect.objectContaining({ component: 'stopStartRecurringGiftsModal' }),
      );
