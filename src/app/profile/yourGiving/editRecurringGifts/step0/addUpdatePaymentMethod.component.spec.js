  ...
  describe('step 0 addUpdatePaymentMethod', () => {
    ...
    describe('loadDonorDetails', () => {
      it('should load mailing address', () => {
        jest.spyOn(self.controller.profileService, 'getDonorDetails').mockReturnValue(Observable.of({ mailingAddress: { streetAddress: 'Some street address'} }));
      ...
      it('should log an error on failure', () => {
        jest.spyOn(self.controller.profileService, 'getDonorDetails').mockReturnValue(Observable.throw('some error'));
        self.controller.loadDonorDetails();
    ...
    describe('onPaymentFormStateChange', () => {
      ...
      it('should save a new payment method to the profile service', () => {
        jest.spyOn(self.controller.profileService, 'addPaymentMethod').mockReturnValue(Observable.of('payment method response'));
      ...
      it('should handle an error saving a new payment method to the profile service', () => {
        jest.spyOn(self.controller.profileService, 'addPaymentMethod').mockReturnValue(Observable.throw({ data: 'payment method error' }));
        self.controller.onPaymentFormStateChange({ state: 'loading', payload: 'some payment method' });
      ...
      it('should save an existing payment method to the profile service', () => {
        ...
        jest.spyOn(self.controller.profileService, 'updatePaymentMethod').mockReturnValue(Observable.of(null));
        self.controller.onPaymentFormStateChange({ state: 'loading', payload: 'some payment method' });
      ...
      it('should handle an error saving an existing payment method to the profile service', () => {
        self.controller.paymentMethod = { self: { uri: 'paymentMethodUri'} };
        jest.spyOn(self.controller.profileService, 'updatePaymentMethod').mockReturnValue(Observable.throw({ data: 'payment method error' }));
        self.controller.onPaymentFormStateChange({ state: 'loading', payload: 'some payment method' });
