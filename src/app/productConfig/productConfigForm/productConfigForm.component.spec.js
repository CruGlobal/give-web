...
describe('product config form component', function () {
  ...
  describe('$onInit', () => {
    ...
    it('should default effectiveMinimumCustomAmount when minimumCustomAmount is not supplied', () => {
      jest.spyOn($ctrl.$rootScope, '$on').mockImplementation(() => {});
      $ctrl.minimumCustomAmount = undefined;

      $ctrl.$onInit();

      expect($ctrl.effectiveMinimumCustomAmount).toEqual(1);
    });

    it('should use minimumCustomAmount for effectiveMinimumCustomAmount when supplied', () => {
      jest.spyOn($ctrl.$rootScope, '$on').mockImplementation(() => {});
      $ctrl.minimumCustomAmount = 25;

      $ctrl.$onInit();

      expect($ctrl.effectiveMinimumCustomAmount).toEqual(25);
    });
  });
  ...
  describe('loadData', () => {
    beforeEach(() => {
      jest
        .spyOn($ctrl.designationsService, 'productLookup')
        .mockReturnValue(Observable.of('product data'));
      ...
      jest
        ...
        .mockReturnValue(Observable.of('2016-10-02'));

      jest
        .spyOn($ctrl.designationsService, 'suggestedAmounts')
        .mockReturnValue(
          Observable.of([{ amount: 5 }, { amount: 10 }, { amount: 0 }, {}]),
        );

      jest
        .spyOn($ctrl.designationsService, 'givingLinks')
        .mockReturnValue(Observable.of([]));
      jest
        .spyOn($ctrl.analyticsFactory, 'giveGiftModal')
        .mockReturnValue(() => {});
    ...
    it('should get productData, nextDrawDate, suggestedAmounts and givingLinks', () => {
      ...
      expect(
        $ctrl.brandedAnalyticsFactory.beginCheckout,
    ...
    it('should called beginCheckout in branded checkout', () => {
      ...
      $ctrl.loadData();
      expect($ctrl.brandedAnalyticsFactory.beginCheckout).toHaveBeenCalledWith(
        $ctrl.productData,
      );
    });

    it('should not use suggested amounts if they are not provided', () => {
      $ctrl.designationsService.suggestedAmounts.mockReturnValue(
        Observable.of([]),
      );
      $ctrl.loadData();
    ...
    it('should show givingLinks if present', () => {
      $ctrl.designationsService.givingLinks.mockReturnValue(
        Observable.of([
          {
            name: 'Name',
    ...
    it('should handle an error loading data', () => {
      $ctrl.designationsService.productLookup.mockReturnValue(
      ...
      expect($ctrl.$log.error.logs[0]).toEqual([
        'Error loading data for product config form',
        'some error',
      ]);
      expect($ctrl.loading).toEqual(false);
    });
  ...
  describe('loadData but suggestedAmounts only has 0 amounts or empty objects', () => {
    beforeEach(() => {
      jest
        .spyOn($ctrl.designationsService, 'suggestedAmounts')
        .mockReturnValue(Observable.of([{ amount: 0 }, {}]));
    });
  ...
  describe('addCustomValidators()', () => {
    beforeEach(() => {
      ...
      $ctrl.effectiveMinimumCustomAmount = 1;
    });
    ...
    it('should create validators', () => {
      ...
      expect($ctrl.itemConfigForm.amount.$validators.minimum('0.9')).toBe(
        false,
      );

      expect(
        $ctrl.itemConfigForm.amount.$validators.maximum('9999999.99'),
      ).toBe(true);
      expect($ctrl.itemConfigForm.amount.$validators.maximum('10000000')).toBe(
        false,
      );
    ...
    it('should enforce a configured minimumCustomAmount', () => {
      $ctrl.customInputActive = true;
      $ctrl.effectiveMinimumCustomAmount = 25;
      $ctrl.addCustomValidators();
    ...
    it("should pass validation in any 'bad' case", () => {
      ...
      expect($ctrl.itemConfigForm.amount.$validators.minimum('dlksfjs')).toBe(
        true,
      );
      expect(
        $ctrl.itemConfigForm.amount.$validators.maximum('4542452454524.99'),
      ).toBe(true);
  ...
  describe('changeFrequency()', () => {
    beforeEach(() => {
      jest
        .spyOn($ctrl.designationsService, 'productLookup')
        .mockReturnValue(Observable.of({ frequency: 'NA' }));
    ...
    it('changes product frequency', () => {
      ...
      expect($ctrl.designationsService.productLookup).toHaveBeenCalledWith(
        '/a',
        true,
      );
      expect($ctrl.itemConfigForm.$setDirty).toHaveBeenCalled();
      expect($ctrl.productData).toEqual({ frequency: 'NA' });
      expect($ctrl.updateQueryParam).toHaveBeenCalledWith({
  ...
  describe('changeAmount()', () => {
    ...
    it('should clear cover fees if we are not explicitly retaining them and the amount changed', () => {
      jest
        .spyOn($ctrl.orderService, 'clearCoverFees')
        .mockImplementation(() => {});
  ...
  describe('saveGiftToCart()', () => {
    ...
    describe('testing transaction flag', () => {
      ...
      it('saves flag with no comment', () => {
        ...
        expect(
          $ctrl.brandedAnalyticsFactory.saveTestingTransaction,
    ...
    function testSaving(isEdit) {
      ...
      const operationArgs = isEdit
        ? [
            'uri',
            'items/crugive/<some id>',
            { AMOUNT: 85, priceWithFees: '$87.05' },
          ]
        : [
            'items/crugive/<some id>',
            { AMOUNT: 85, priceWithFees: '$87.05' },
      ...
      beforeEach(() => {
        $ctrl.isEdit = isEdit;
        jest
          .spyOn($ctrl.cartService, operation)
          .mockReturnValue(Observable.of({ self: { uri: 'uri' } }));
      ...
      it('should still submit the gift if the form is not dirty', () => {
        ...
        expect($ctrl.submittingGift).toEqual(false);
        expect($ctrl.cartService[operation]).toHaveBeenCalledWith(
          ...operationArgs,
        );
      ...
      it('should submit a gift successfully', () => {
        ...
        expect($ctrl.errorSavingGeneric).toEqual(false);
        $ctrl.isEdit
          ? expect($ctrl.analyticsFactory.cartAdd).not.toHaveBeenCalled()
          : expect($ctrl.analyticsFactory.cartAdd).toHaveBeenCalledWith(
              $ctrl.itemConfig,
              $ctrl.productData,
            );
      });
      ...
      it('should handle an error submitting a gift', () => {
        ...
        expect($ctrl.$log.error.logs[0]).toEqual([
          'Error adding or updating item in cart',
          'some error',
        ]);
      });
      ...
      it('should handle an error when saving a duplicate item', () => {
        $ctrl.cartService[operation].mockReturnValue(
          Observable.throw({
            data: 'Recurring gift to designation: 0671540 on draw day: 14 is already in the cart',
          }),
        );
        $ctrl.itemConfigForm.$dirty = true;
        $ctrl.saveGiftToCart();
  ...
  describe('shouldShowForcedUserToLogoutError', () => {
    it('should call $window.sessionStorage', () => {
      $ctrl.$onInit();
      expect($ctrl.$window.sessionStorage.getItem).toHaveBeenCalledWith(
        forcedUserToLogout,
      );
