...
describe('cart', () => {
  ...
  describe('$onInit()', () => {
    it('should call loadCart()', () => {
      ...
      jest
        .spyOn(self.controller.analyticsFactory, 'cartView')
        .mockImplementation(() => {});
  ...
  describe('loadCart()', () => {
    ...
    it('should preserve the order', () => {
      self.controller.cartService.get.mockReturnValue(
        Observable.of({ items: [{ code: '1' }, { code: '2' }, { code: '3' }] }),
      );
      self.controller.loadCart(true);

      self.controller.cartService.get.mockReturnValue(
        Observable.of({
          items: [{ code: '3' }, { code: '2' }, { code: '1' }, { code: '4' }],
        }),
      );
      self.controller.loadCart(true);

      expect(self.controller.cartData).toEqual({
        items: [{ code: '4' }, { code: '1' }, { code: '2' }, { code: '3' }],
      });
    });
  ...
  describe('removeItem()', () => {
    beforeEach(() => {
      ...
      jest
        .spyOn(self.controller.analyticsFactory, 'cartRemove')
        .mockImplementation(() => {});
    ...
    it('should remove item from cart', () => {
      jest.spyOn(self.controller, 'loadCart').mockImplementation(() => {});
      self.controller.cartService.deleteItem.mockReturnValue(
        Observable.of('data'),
      );
      self.controller.removeItem(self.controller.cartData.items[0]);

      expect(self.controller.cartService.deleteItem).toHaveBeenCalledWith(
        'uri1',
      );
      ...
      expect(self.controller.analyticsFactory.cartRemove).toHaveBeenCalledWith({
        removing: true,
        uri: 'uri1',
      });
    });
    ...
    it('should handle an error removing an item', () => {
      ...
      expect(self.controller.cartData.items).toEqual([
        { uri: 'uri1', removingError: true },
        { uri: 'uri2' },
      ]);
      expect(self.controller.$log.error.logs[0]).toEqual([
      ...
      expect(
        self.controller.analyticsFactory.cartRemove,
  ...
  describe('editItem()', () => {
    beforeEach(() => {
      ...
      jest.spyOn(self.controller, 'loadCart').mockImplementation(() => {});
      jest
        .spyOn(self.controller.productModalService, 'configureProduct')
        .mockImplementation(self.controller.callback);
      self.controller.cartData = {
        items: [
          { code: '0123456', config: 'some config', uri: 'uri1' },
          { uri: 'uri2' },
        ],
      };
    });
    ...
    it('should call a modal and reload cart of successful edit', () => {
      ...
      expect(
        self.controller.productModalService.configureProduct,
      ).toHaveBeenCalledWith('0123456', 'some config', true, 'uri1');
      expect(self.controller.loadCart).toHaveBeenCalledWith(true);
    });
  });
  ...
  describe('setContinueBrowsingUrl()', () => {
    ...
    it('set continue browsing url', () => {
      self.controller.$document[0].referrer =
        'https://give-stage2.cru.org/page-to-give-more';
      self.controller.setContinueBrowsingUrl();

      expect(self.controller.continueBrowsingUrl).toEqual(
        'https://give-stage2.cru.org/page-to-give-more',
      );
    });
    ...
    it('skip if not give url', () => {
      self.controller.$document[0].referrer =
        'https://www.cru.org/another-page';
      self.controller.setContinueBrowsingUrl();
    ...
    it('remove giving modal params', () => {
      self.controller.$document[0].referrer =
        'https://give-stage2.cru.org/page-to-give-more?modal=give-gift';
      self.controller.setContinueBrowsingUrl();
