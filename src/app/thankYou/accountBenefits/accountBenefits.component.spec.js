...
describe('thank you', function () {
  describe('accountBenefits', function () {
    ...
    beforeEach(inject(function ($componentController) {
      ...
      jest
        .spyOn($ctrl.orderService, 'retrieveLastPurchaseLink')
        .mockReturnValue(`/purchases/crugive/${lastPurchaseId}`);
    }));
    ...
    describe('$onChanges', () => {
      ...
      it("is visible when registration-state is 'MATCHED'", () => {
        $ctrl.$onChanges({
          donorDetails: { currentValue: { 'registration-state': 'MATCHED' } },
        });
      ...
      it("is not visible when registration-state is 'COMPLETED'", () => {
        $ctrl.$onChanges({
          donorDetails: { currentValue: { 'registration-state': 'COMPLETED' } },
        });
    ...
    describe('openModal', () => {
      ...
      beforeEach(inject((_$q_, _$rootScope_) => {
        ...
        jest
          .spyOn($ctrl.sessionModalService, 'accountBenefits')
          .mockReturnValue(deferred.promise);
        jest
          .spyOn($ctrl.sessionModalService, 'registerAccount')
          .mockReturnValue(deferred.promise);
        jest
      ...
      it('shows userMatch modal to users who are authenticated but need to complete donor matching', (done) => {
        $ctrl.sessionService.oktaIsUserAuthenticated.mockReturnValue(
          Observable.from([true]),
        );
        $ctrl.donorDetails = { 'registration-state': 'MATCHED' };
        $ctrl.openModal();

        expect($ctrl.sessionModalService.userMatch).toHaveBeenCalledWith(
          lastPurchaseId,
        );
        deferred.resolve();
        $rootScope.$digest();
      ...
      it('shows registerAccount modal to users who are authenticated but need to register for a donor account', () => {
        ...
        expect($ctrl.sessionModalService.registerAccount).toHaveBeenCalledWith({
          lastPurchaseId,
        });
        deferred.resolve();
        $rootScope.$digest();
      ...
      it("shows accountBenefits modal to users who aren't authenticated when defaultToAccountBenefits is true", () => {
        ...
        expect($ctrl.sessionModalService.accountBenefits).toHaveBeenCalledWith(
          lastPurchaseId,
        );
        deferred.resolve();
        $rootScope.$digest();
      ...
      it('should not show accountBenefits modal if purchase link is missing', () => {
        ...
        expect(
          $ctrl.sessionModalService.accountBenefits,
        ).not.toHaveBeenCalled();
      });
      ...
      it('should log an error', () => {
        ...
        expect($ctrl.$log.error.logs[0]).toEqual([
          'Failed checking if user is authenticated',
          error,
        ]);
      });
    });
