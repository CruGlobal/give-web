...
- `campaign-page` - the campaign page you would like to use, used for suggested amounts - _Optional_
- `campaign-code` - the campaign code you would like to use - _Optional_
...
- `day` - for recurring gifts this defaults the gift's day of the month - _Optional_ - can be `1` to `28`
- `default-payment-type` - if set to `creditCard`, the credit card form will be shown by default instead of bank account - _Optional_
- `donor-details` - name of the window variable containing default values for donor's name and contact info - _Optional_ - should be in this format:
  ```javascript
  window.donorDetails = {
...
To set up a branded checkout for your ministry, there are a number of steps that must be completed. Please follow the instructions in this Google Doc:
https://docs.google.com/document/d/1in5Nw2T2c0Umm-3npFhIydHml53lAG6Z_9wCF5k2_8M/edit.
